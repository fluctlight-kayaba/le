...
fn main() {
...

	{
		...
		let timeout_function = v8::FunctionTemplate::new(scope, timeout);
		let timeout_str = v8::String::new(scope, "setTimeout").unwrap();

		global_object.set(log_str.into(), log_function.into());
		global_object.set(timeout_str.into(), timeout_function.into());

		let context = v8::Context::new_from_template(scope, global_object);
		let scope = &mut v8::ContextScope::new(scope, context);

		let source = fs::read_to_string("src/script.js").expect("Could not scripting read source code");
...
fn timeout(
	...
	let callback_obj = args.get(0).to_object(scope).unwrap();
	// cb.is_function()
	println!("timeout! {:?}", callback_obj.is_function());
}
